...
Game::Game() :
    ...
    Player* lastPlayerWhoActed = nullptr;
...
void Game::nextTurn() {
    ...
    Player* previous = players[currentTurnIndex];  
    ...
    // Reset last arrested player (to allow arresting them again in future turns)
    if (previous != p) {
        lastArrested = nullptr;
    }

    lastPlayerWhoActed = p;
}
        ...
        for (Player* p : players) {
            if (p->isActive() && p != actor && p->canBlockCoup()) {
                if (p->getRole() == "General"){
                    std::cout << p->getName() << "You need to pay 5 coins to block the coup!" << "\n";
                }
